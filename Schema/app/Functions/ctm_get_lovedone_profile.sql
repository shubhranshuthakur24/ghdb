CREATE OR REPLACE FUNCTION app.ctm_get_lovedone_profile(user_id integer, loved_one_id integer)
 RETURNS TABLE(loved_one_data json)
 LANGUAGE plpgsql
AS $function$
declare
cgid integer := (select care_giverid from app.care_team_member ctm
	left join app.care_team on ctm.team_id = app.care_team.team_id 
	where ctm.userid = user_id
) ;
cg_userid integer := (select userid from app.care_giver where care_giverid = cgid);
begin
	return QUERY
	select
