CREATE OR REPLACE FUNCTION app.cg_get_saved_toolkit(user_id integer, data_limit integer, data_offset integer)
		...
		from(with cte as (
				...
				select 
					...
					(select is_saved from (
						select (select saved_toolkit from app.care_giver where userid=user_id) @>  array[unnest(array(select toolkitid from app.toolkit))] as is_saved, 
