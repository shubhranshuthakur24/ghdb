CREATE OR REPLACE FUNCTION app.get_assessment(user_id integer)
...
begin
	...
	from 
		...
		union all
		select res.question,res.answer from (select unnest(array['Does your loved one have private health insurance?' ,
		'Notes',
		...
		unnest((select array[primary_insurance::text, notes, long_term_insurance::text, priorities::text, more_info::text] 
