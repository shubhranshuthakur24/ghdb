CREATE OR REPLACE FUNCTION app.cg_create_team_referral(user_id integer, role_id integer, name character varying, email character varying)
 RETURNS text
 LANGUAGE plpgsql
AS $function$
declare
...
--	if not found then 
	  INSERT INTO app.team_referral 
    	(upcode, created_by, roleid,name,email)
      VALUES(up_code, user_id, role_id,name,email);
--    end if;
