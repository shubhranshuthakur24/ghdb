CREATE OR REPLACE FUNCTION app.cg_get_toolkit(user_id integer, toolkit_type_id integer)
...
row_to_json(t) as toolkit
	from (select 
		...
		select (select saved_toolkit from app.care_giver where userid=user_id) @>  array[unnest(array(select toolkitid from app.toolkit order by toolkitid))] as is_saved, 
